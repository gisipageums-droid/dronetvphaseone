      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {categoryOptions.map(({ value, icon: Icon, description }) => (
            <label
              key={value}
              className={`flex flex-col items-center p-6 border-2 rounded-xl cursor-pointer transition-all hover:shadow-lg ${formData.companyCategory.includes(value)
            ...
            >
              <input
                ...
                className="sr-only"
              />
              <Icon className={`w-12 h-12 mb-4 ${formData.companyCategory.includes(value) ? 'text-blue-600' : 'text-slate-400'
                }`} />
              <h3 className={`text-xl font-bold mb-2 ${formData.companyCategory.includes(value) ? 'text-blue-900' : 'text-slate-700'
                }`}>
                {value}
              </h3>
              <p className={`text-sm text-center ${formData.companyCategory.includes(value) ? 'text-blue-700' : 'text-slate-500'
                }`}>
                {description}
              </p>
            </label>
