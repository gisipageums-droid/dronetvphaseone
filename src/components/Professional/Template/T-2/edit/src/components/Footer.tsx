...
export function Footer({ footerData, onStateChange }: FooterProps) {
  ...
  // Ref for onStateChange to prevent infinite loops
  ...
  const defaultFooterData: FooterData = {
    ...
    socialLinks: [
      ...
      { icon: "Mail", label: "Email", href: "#", color: "#EA4335" },
    ],
  ...
  const [data, setData] = useState<FooterData>(footerData || defaultFooterData);
  ...
  const scheduleAutoSave = useCallback(
    (updatedData: FooterData) => {
      ...
      // Schedule new auto-save (2 second delay)
