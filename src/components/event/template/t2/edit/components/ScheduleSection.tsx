...
interface ScheduleProps {
  scheduleData?: any; // Changed to any to accept both formats
...
const convertDaysArrayToObject = (daysArray: any[]): { [key: number]: DayData } => {
  ...
  daysArray.forEach((day, index) => {
    ...
    daysObject[dayNumber] = {
      label: day.label,
      date: day.date.includes(' - ') ? day.date.split(' - ')[1] : day.date, // Extract just the date part
...
export function ScheduleSection({ scheduleData, onStateChange, userId, eventId, templateSelection }: ScheduleProps) {
