...
// // Custom Name with Title Component for Speakers
...
//                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//                   {/* Updated Name with Title Component */}
//                   <div className="md:col-span-2">
//                     <NameWithTitle
//                       label="Name"
//                       titleValue={speaker.title || ''}
//                       nameValue={speaker.name || ""}
//                       onTitleChange={(value) => handleChange("speakers", index, "title", value)}
//                       onNameChange={(value) => handleChange("speakers", index, "name", value)}
//                       required
//                       namePlaceholder="Full name of the speaker"
...
interface Speaker {
  ...
  day: string;
  sequence: number;
  prefix?: string; // Changed from title to prefix
}
...
// Custom Name with Prefix Component for Speakers
interface NameWithPrefixProps {
  label: string;
  prefixValue: string;
  nameValue: string;
...
const NameWithPrefix: React.FC<NameWithPrefixProps> = ({
  label,
  prefixValue,
  nameValue,
  onPrefixChange,
  ...
  // Set default value to "Mr" if no value is provided
  const actualValue = prefixValue || "Mr";
          ...
          <div className="space-y-4">
            {speakers.map((speaker: Speaker, index: number) => (
              <div key={index} className="p-6 bg-white rounded-lg border border-amber-200 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Updated Name with Prefix Component */}
                  <div className="md:col-span-2">
                    <NameWithPrefix
                      label="Name"
                      prefixValue={speaker.prefix || 'Mr'} // Default to 'Mr' if undefined
                      nameValue={speaker.name || ""}
                      onPrefixChange={(value) => handleChange("speakers", index, "prefix", value)}
                      onNameChange={(value) => handleChange("speakers", index, "name", value)}
                      required
                      namePlaceholder="Full name of the speaker"
                      error={errors[`speakers-${index}-name`]}
