...
const Step6: React.FC<Step6Props> = ({ className = '', onSubmit }) => {
  ...
  // Debounced context update to prevent spam
  useEffect(() => {
  ...
  }, [documents, showFullText, onSubmit]);
  ...
  const extractTextFromFile = useCallback(async (file: File): Promise<string> => {
    ...
    if (fileType.includes('text') || file.name.endsWith('.txt')) return await extractTextFromTxt(file);
    throw new Error(`Unsupported file type: ${fileType}`);
  }, [extractTextFromPDF, extractTextFromWord, extractTextFromTxt]);
  ...
  // Utility functions
  const downloadText = useCallback((doc: ExtractedDocument) => {
    const element = document.createElement('a');
    const file = new Blob([doc.extractedText], { type: 'text/plain' });
    element.href = URL.createObjectURL(file);
    element.download = `${doc.name}_extracted.txt`;
  ...
  const deleteDocument = useCallback((id: string) => {
    setDocuments(prev => {
      ...
      if (selectedDocument?.id === id) {
        setSelectedDocument(newDocuments.length > 0 ? newDocuments[0] : null);
      }
      return newDocuments;
    });
  }, [selectedDocument]);
  ...
  const getFileTypeIcon = useCallback((type: string, name: string) => {
    if (type.includes('pdf') || name.endsWith('.pdf')) return '📄';
    if (type.includes('word') || name.endsWith('.docx') || name.endsWith('.doc')) return '📝';
    if (type.includes('text') || name.endsWith('.txt')) return '📋';
    return '📎';
