...
const Profile = ({
  ...
  // Get profile-level directorPrefix from profileData
  const profileDirectorPrefix = profileData?.directorPrefix || "Mrs.";
  ...
  const [contentState, setContentState] = useState(() => {
    if (profileData?.teamMembers) {
      return {
        ...profileData,
        directorPrefix: profileDirectorPrefix,
        teamMembers: profileData.teamMembers.map(member => ({
          ...member,
          prefix: member.prefix || profileDirectorPrefix.toLowerCase().replace(".", "") || "mr"
        }))
      };
    }
    return {
  ...
  const getTeamMemberImage = (member) => {
    ...
    if (member.image && member.image.trim() !== "" && member.image !== null) {
      ...
      if (member.image.includes("unsplash.com") || member.image.includes("images.unsplash")) {
        // It's a placeholder, use local avatar based on prefix
    ...
    // For team members without images, use local avatar based on their prefix
    return getAvatar(member.prefix);
  };
  ...
  // Update function for profile-level directorPrefix
  const updateDirectorPrefix = (value) => {
    setContentState((prev) => ({
      ...prev,
      directorPrefix: value,
    }));
  };
    ...
    setContentState((prev) => ({
      ...
      teamMembers: [
        ...prev.teamMembers,
        {
          ...
          name: "New Member",
          role: "New Role",
          prefix: (prev.directorPrefix || "Mrs.")
            .toLowerCase()
            .replace(".", "") || "mr",
          image: null,
          bio: "Team member bio...",
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Profile-Level Director Prefix Editor (Only in Edit Mode) */}
          {isEditing && (
            <div className="mb-6 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg">
              <label className="block text-sm font-medium mb-2">
                Default Director Prefix (for new team members):
              </label>
              <select
                value={contentState.directorPrefix?.toLowerCase().replace('.', '') || "mrs"}
                onChange={(e) => updateDirectorPrefix(e.target.value)}
                className="border border-gray-300 dark:border-gray-600 bg-transparent px-3 py-2 rounded"
              >
              ...
              <p className="text-xs text-gray-500 mt-1">
                This prefix will be used as default for new team members
              </p>
            </div>
          )}
                ...
                <div className="relative h-60 overflow-hidden">
                  ...
                  {isEditing && (
                    <>
                      ...
                      {/* Prefix Dropdown in top-right corner */}
                      <motion.div
                        animate={{ opacity: [0, 1], scale: [0.8, 1] }}
                        className="absolute mx-2 top-2 right-2 z-50 bg-white/80 p-2 rounded"
                      >
                        <label className="text-xs text-gray-600 mb-1 block">Title:</label>
                        <select
                          value={member.prefix?.toLowerCase().replace('.', '') || "mr"}
                          onChange={(e) => updateTeamMemberField(index, "prefix", e.target.value)}
                          className="text-xs border bg-transparent text-center w-full p-1"
                        >
                          <option value="mr">Mr.</option>
                          <option value="ms">Ms.</option>
                          <option value="mrs">Mrs.</option>
