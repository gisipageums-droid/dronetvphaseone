...
export default function Footer({
  ...
  const initialData = {
    brand: {
      name: footerLogo?.name || "Innovative Labs",
      ...
      logoUrl: footerLogo?.logo || "/api/placeholder/32/32",
    },
    contact: {
  ...
  const [footerContent, setFooterContent] = useState(initialData);
  const [tempContent, setTempContent] = useState(initialData);
