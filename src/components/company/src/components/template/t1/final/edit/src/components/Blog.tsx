...
export default function Blog({
  ...
  const [tempContent, setTempContent] = useState<BlogContent>(defaultContent);
  ...
  const tempContentRef = useRef(tempContent);
  ...
  }, [tempContent, pendingImages, isEditing]);
  ...
  const getCroppedImg = async (imageSrc: string, pixelCrop: CropArea, rotation = 0) => {
  ...
  const uploadImageToAWS = async (file: File, postId: number): Promise<string | null> => {
  ...
  const handleAutoSave = async () => {
    ...
    try {
      ...
      let updatedContent = { ...tempContentRef.current };
      const uploadedImages: Record<number, string> = {};

      for (const [postIdStr, file] of Object.entries(pendingImagesRef.current)) {
        const postId = parseInt(postIdStr);
        const imageUrl = await uploadImageToAWS(file, postId);
        
        if (imageUrl) {
          uploadedImages[postId] = imageUrl;
          // Update the post image in our local copy
          updatedContent.posts = updatedContent.posts.map((post) =>
            post.id === postId ? { ...post, image: imageUrl } : post
          );
        }
      }
      ...
      // Update content with any changes and uploaded images
      ...
      if (imageUrl) {
        ...
        setTempContent((prev) => ({
          ...
          posts: prev.posts.map((post) =>
            post.id === cropIndex ? { ...post, image: imageUrl } : post
          ),
        }));

        // Also update main content for immediate consistency
        setContent((prev) => ({
          ...prev,
          posts: prev.posts.map((post) =>
            post.id === cropIndex ? { ...post, image: imageUrl } : post
  ...
  // Fixed: Simplified EditableText component without auto-save interference
  const EditableText = useMemo(
    () =>
      ({
        ...
        const baseClasses =
          "w-full bg-white/80 border-2 border-dashed border-blue-300 rounded focus:border-blue-500 focus:outline-none transition-colors duration-200 p-2";
  ...
  // Fixed: Update functions that properly handle state without auto-save interference
  const updateBlogField = useCallback((id: number, field: string, value: string) => {
    setTempContent((prev) => ({
      ...prev,
  ...
  return (
    <>
      ...
      >
        ...
        {isEditing && (
          <motion.div
            className="absolute top-4 left-4 flex items-center gap-2 text-xs text-gray-500"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          >
            {isSaving ? (
                          ...
                          >
                            <label className="flex items-center gap-2 bg-white/90 dark:bg-gray-800/90 p-2 rounded shadow cursor-pointer">
                              ...
                              <input
                                ...
                                accept="image/*"
                                className="hidden"
                                onChange={(e) => handleImageUpload(b.id, e)}
                              />
                            </label>
                          </motion.div>
                        )}
