...
export default function CompanyProfile({ profileData }) {
  ...
  // Mock data
  const companyName = profileData?.companyName || "Innovative Labs";
  const establishedYear = profileData?.establishedYear || 2015;
  const growthThisYear = profileData?.growthThisYear || 42;
  const satisfiedCustomers = profileData?.satisfiedCustomers || 20;
  const teamSize = profileData?.teamSize || 150;
  const projectsDelivered = profileData?.projectsDelivered || 25;
          ...
          <div className="space-y-8">
            ...
            >
              ...
              <p className="text-lg text-gray-700 mt-4 max-w-xl">
                Founded in <strong>{establishedYear}</strong>,{" "}
                {profileData?.companyName} || "Innovative Labs" is a global
                leader in delivering cutting-edge tech solutions that drive
