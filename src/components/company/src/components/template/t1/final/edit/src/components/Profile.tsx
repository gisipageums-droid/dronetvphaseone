...
export default function EditableCompanyProfile({
  ...
  const uploadImageToAWS = async (file: File): Promise<string | null> => {
  ...
  const autoSave = useCallback(async () => {
    ...
    try {
      ...
      // Update content with any changes
  ...
  const updateTempContent = useCallback((field, value) => {
    ...
    scheduleAutoSave(); // Trigger auto-save on content update
  }, [scheduleAutoSave]);
  ...
  const updateCoreValue = useCallback((index, value) => {
    ...
    scheduleAutoSave(); // Trigger auto-save on core value update
  }, [scheduleAutoSave]);
  ...
  const addCoreValue = useCallback(() => {
    ...
    scheduleAutoSave(); // Trigger auto-save for new core value
  }, [scheduleAutoSave]);
  ...
  const removeCoreValue = useCallback((index) => {
    ...
    scheduleAutoSave(); // Trigger auto-save for core value removal
  }, [scheduleAutoSave]);
            ...
            >
              ...
              <div className="rounded-3xl overflow-hidden shadow-xl border border-yellow-100 w-full max-w-[900px] cursor-pointer" onClick={() => { if (isEditing) fileInputRef.current?.click(); }}>
                <img
                  ...
                  className="block w-full h-auto max-h-[75vh] object-contain scale-110"
